...
interface props{
    loadedItems: PostCollection | undefined
    loading?:boolean
    rows?: string
    width?: '4xl' | '2xl' | '3xl' | '5xl' | '6xl' | '7xl'
}

export const MultiBlog = (props:props) => {
    
    const gridCols = props.rows ?? 'sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2';
    const maxW = props.width ?? '4xl';
    ...
    return(
        <div className={`mx-auto max-w-${maxW} grid grid-cols-1 gap-5 ${gridCols}`}>
            {!props.loading &&
                ...
                props.loadedItems ? 
                    props.loadedItems.postsConnection.edges.map((edge) => (
